...
<template>
  <div class="flex items-center">
    ...
    <ElBreadcrumb separator="/">
      ...
      <ElBreadcrumbItem class="text-size-12px">宅基地</ElBreadcrumbItem>
    </ElBreadcrumb>
  </div>
  <WorkContentWrap>
    ...
    <div class="table-wrap">
      <div class="flex items-center justify-between pb-12px">
        <div class="table-left-title">宅基地报表</div>
      ...
      >
        ...
        <el-table-column prop="area" label="户型（m²）" align="center">
          <template #default="{ row }">
            {{
              homesteadAreaSize
                .filter((items) => items.id == row.area)
...
import {
  getSiteHomesteadStatisticsListApi,
  exportSiteHomesteadStatisticsApi
...
import {
  resettleAreas,
  homesteadAreaSize
} from '@/views/Workshop/ImmigrantImplement/DataFill/config'
...
const getProHouseReportList = () => {
  ...
  getSiteHomesteadStatisticsListApi(params).then(
    (res) => {
...
const onExport = async () => {
  ...
  const res = await exportSiteHomesteadStatisticsApi(params)
