    ...
    <div class="table-wrap">
      ...
      <div class="row-cont">
        ...
        <div class="row">
          ...
          <div class="value">{{ detail.receipt }}</div>
        </div>

        <div class="row">
          <div class="label">凭证：</div>
          <div class="value">
            <div class="img-list" v-if="detail.receiptPic && detail.receiptPic.length">
              <div
                class="img-box"
                @click="viewImg(item.url)"
                v-for="(item, index) in detail.receiptPic"
                :key="index"
              >
...
import { getFundEntryByIdApiLegal } from '@/api/fundManage/fundEntry-service'
...
onMounted(() => {
  ...
  getFundEntryByIdApiLegal({ id }).then((res) => {
    if (res) {
      if (res.receiptPic) {
        res.receiptPic = JSON.parse(res.receiptPic as string)
      }
      detail.value = res
    }
  })
