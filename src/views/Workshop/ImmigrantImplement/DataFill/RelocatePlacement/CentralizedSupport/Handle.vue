...
const relocatePic = ref<FileItemType[]>([])
...
const rules = reactive<FormRules>({
  relocateCompleteTime: [{ required: true, message: '请选择', trigger: 'blur' }]
})
...
const onSubmit = debounce((formEl) => {
  formEl?.validate((valid: any) => {
    if (valid) {
      if (!form.value.relocateCompleteTime) {
        ElMessage.error('请选择完成时间')
        return
      } else if (!relocatePic.value.length) {
        ElMessage.error('请上传集中供养凭证')
        return
      } else {
        const data: any = {
          ...form.value,
          relocatePic: JSON.stringify(relocatePic.value || [])
        }
        loading.value = true
        submit(data)
      }
