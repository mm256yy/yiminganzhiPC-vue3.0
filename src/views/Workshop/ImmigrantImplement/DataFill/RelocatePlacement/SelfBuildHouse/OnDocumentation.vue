  ...
  >
    ...
    >
      <ElRow>
        <ElCol :span="24">
          <div class="col-wrapper">
            <div class="col-label-required">自建房告知单（盖章/签字）：</div>
      ...
      <ElRow>
        <ElCol :span="24">
          <div class="col-wrapper">
            <div class="col-label"> 自建房验收告知单（盖章/签字）： </div>
            <div class="card-img-list">
              <ElUpload
                action="/api/file/type"
                :data="{
...
const buildOneselfPic = ref<FileItemType[]>([]) // 交房协议文件列表
const buildOneselfCheckPic = ref<FileItemType[]>([]) // 购房测算表文件列表
const buildOneselfOtherPic = ref<FileItemType[]>([]) // 其他附件列表
...
const initData = () => {
  getDocumentationApi(props.doorNo).then((res: any) => {
    if (res) {
      form.value = { ...res }
      if (form.value.buildOneselfPic) {
        buildOneselfPic.value = JSON.parse(form.value.buildOneselfPic)
      }

      if (form.value.buildOneselfCheckPic) {
        buildOneselfCheckPic.value = JSON.parse(form.value.buildOneselfCheckPic)
      }

      if (form.value.buildOneselfOtherPic) {
        buildOneselfOtherPic.value = JSON.parse(form.value.buildOneselfOtherPic)
      }
    }
  })
}
...
const handleFileList = (fileList: UploadFiles, type: string) => {
  ...
  if (type === 'buildOneself') {
    buildOneselfPic.value = list
  } else if (type === 'buildOneselfCheck') {
    buildOneselfCheckPic.value = list
  } else if (type === 'buildOneselfOther') {
    buildOneselfOtherPic.value = list
  }
}

// 文件上传
const uploadFileChange1 = (_response: any, _file: UploadFile, fileList: UploadFiles) => {
  handleFileList(fileList, 'buildOneself')
}

const uploadFileChange2 = (_response: any, _file: UploadFile, fileList: UploadFiles) => {
  handleFileList(fileList, 'buildOneselfCheck')
}

const uploadFileChange3 = (_response: any, _file: UploadFile, fileList: UploadFiles) => {
  handleFileList(fileList, 'buildOneselfOther')
}
