      ...
      <ElRow>
        <ElCol :span="24">
          <div class="col-wrapper">
            <div class="col-label-required"> 动迁安置协议（盖章/签字）: </div>
...
const agreementPic = ref<FileItemType[]>([]) // 生产安置确认单文件列表
const agreementOtherPic = ref<FileItemType[]>([]) // 其他附件列表
...
const initData = () => {
  getDocumentationApi(props.doorNo).then((res: any) => {
    if (res) {
      form.value = { ...res }
      if (form.value.agreementPic) {
        agreementPic.value = JSON.parse(form.value.agreementPic)
      }

      if (form.value.agreementOtherPic) {
        agreementOtherPic.value = JSON.parse(form.value.agreementOtherPic)
      }
    }
  })
}
    ...
    if (valid) {
      if (!agreementPic.value.length) {
        ...
        let params: any = {
          ...form.value,
          doorNo: props.doorNo,
          agreementPic: JSON.stringify(agreementPic.value || []), // 生产安置确认单
          agreementOtherPic: JSON.stringify(agreementOtherPic.value || []) // 其他附件
        }
        submit(params)
      }
    } else {
...
const handleFileList = (fileList: UploadFiles, type: string) => {
  ...
  if (type === 'produceVerify') {
    agreementPic.value = list
  } else if (type === 'produceOther') {
    agreementOtherPic.value = list
  }
}
